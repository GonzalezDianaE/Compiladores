...
void generate(item *func){    
	if (func->type == VAR){
		func->type = VARGLOBAL;
		OpThreeDir *varGlobal = (OpThreeDir *) malloc(sizeof(OpThreeDir));
		varGlobal->instr = IC_VARGLOBAL;
		varGlobal->result = func;
		varGlobal->oper1 = NULL;
		varGlobal->oper2 = NULL;
		insertOperation (varGlobal);  
	}else{
...
item *setVar(node *tree){
		if (tree->content!=NULL){
				...
				if (type==VAR || type==PARAMETER || type==VARGLOBAL || type==CONSTANT){
						return tree->content;
				}
				else{
		...
		while(aux!=NULL){
			...
			switch (operation->instr) {
				...
				case IC_ASSIGN : {
					...
					printf("%s           ",operation->oper1->name);
					printf("%s      ",operation->oper2->name);
					if (operation->result->type == CONSTANT){
...
void generateFunctionCall(node *tree){
	...
	if(tree->content->type==FUNCTION_CALL_P){
		...
		symbol *aux6; //guarda la direccion de memoria del 6 parameter
		aux->next = NULL;
		symbol *headParams = aux;
		if(currentParam->next!=NULL){
			currentParam = currentParam->next;
			while (currentParam!=NULL){
				i++;
				if (i<=7){
					aux->next = (symbol *) malloc(sizeof(symbol));
					aux = aux->next;
					aux->content = setVar(currentParam->param);
					aux->next = NULL;
					if (i==6){
						aux6 = aux;
				}
		...
		if (i>6 && ((i%2)!=0)){
			aux = (symbol *) malloc(sizeof(symbol));
			aux->content = (item *) malloc(sizeof(item));
			//SI ES DE TIPO REGISTER SE PUSHEA EBX
			strcpy(aux->content->name,"RBX");
			aux->content->type = REGISTER;
			aux->next = aux6->next;
			aux6->next=aux;
		}
		aux=headParams;
		i=0;
		OpThreeDir *loadParam;
		item *result;
		while (aux->next!=NULL){
			i++;
			aux = aux->next;
